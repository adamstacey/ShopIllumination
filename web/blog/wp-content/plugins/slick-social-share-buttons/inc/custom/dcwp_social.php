...
class dc_jqslicksocial_buttons {
	...
	function dcssb_check_category(){
		...
		$check = '0';
		$catId = '';
		$categories = $this->get_dcssb_default('exclude_category');
	...
	function slick_buttons(){
		...
		$dcssb = '<div id="dc-dcssb"><ul id="nav-dcssb" class="'.$direction.'">';
		...
		$dcssb .= '</ul><div class="clear"></div><div class="dc-corner"><span></span></div></div>';
	...
	function dcssb_inc_facebook($link){
		...
		if($this->get_dcssb_default('incFacebook')){
			...
			if($method == 'xfbml'){
			$button .= '<div id="dcfb-root"></div><script>
	...
	(function() {
		...
		e.async = true;
	document.getElementById("dcfb-root").appendChild(e);
	}());
</script>';
			$button .= '<li id="dcssb-facebook" class="'.$classSize.'"><script src="http://connect.facebook.net/en_US/all.js#xfbml=1"></script><fb:like href="'.$elink.'" send="false" layout="'.$size.'" show_faces="false" font=""></fb:like></li>';
		} else {
			if($classSize == 'size-small'){
				...
				$button .= '<li id="dcssb-facebook" class="'.$classSize.'"><iframe src="http://www.facebook.com/plugins/like.php?app_id='.$appId.'&amp;href='.$elink.'&amp;send=false&amp;layout='.$size.'&amp;width=50&amp;show_faces=false&amp;action=like&amp;colorscheme=light&amp;font&amp;height=60" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:50px; height:60px;" allowTransparency="true"></iframe></li>';
			}
		}
		}
	...
	function dcssb_inc_twitter($link){
		...
		global $post;
		$title = substr(str_replace(array(">","<"),"",get_the_title($post->ID)), 0, 120);
		$button = '';
		$twitterId = '';
	...
	function dcssb_inc_stumble($link){
		...
		if($this->get_dcssb_default('incStumble')){
			...
			$button = '<li id="dcssb-stumble" class="'.$classSize.'">';
			$button .= '<iframe src="http://www.stumbleupon.com/badge/embed/'.$size.'/?url='.$elink.'" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:50px; height: 60px;" allowTransparency="true"></iframe></li>';
	...
	function dcssb_inc_digg($link){
		...
		global $post;
		$elink = urlencode($link);
		$title = substr(urlencode(str_replace(array(">","<"),"",get_the_title($post->ID))), 0, 120);
		$post_by_id = get_post($post->ID, ARRAY_A);
		@$description = strip_tags(substr($post_by_id['post_content'], 0, 350));
		$button = '';
...
s1.parentNode.insertBefore(s, s1);
	...
	function dcssb_inc_buzz($link){
		...
		$image_default = ($image_url == '' ? dc_jqslicksocial_buttons::get_dcssb_default('image_buzz') : $image_url);
	
		$title = substr(str_replace(array(">","<"),"",get_the_title($post->ID)), 0, 120);
		$button = '';
		$size = $this->get_dcssb_default('size_buzz');
		$classSize = $size == 'small-button' || $size == 'small-count' || $size == 'normal-button' || $size == 'link' ? 'size-small' : 'size-box' ;
		
		if($this->get_dcssb_default('incBuzz')){
		
			$button = '<li id="dcssb-buzz" class="'.$classSize.'"><a href="http://www.google.com/buzz/post" class="google-buzz-button" title="Google Buzz" data-message="'.$title.'" data-url="'.$link.'" data-imageurl="'.$image_default.'" data-button-style="'.$size.'"></a>';
			$button .= '<script type="text/javascript" src="http://www.google.com/buzz/api/button.js"></script></li>';
		}
	...
	<meta property="og:site_name" content="'.$name.'"/>';
		...
		';
		$title .= '
			<meta property="og:description" content="'.@strip_tags(str_replace(array("[","]",'"'),"",substr($post_by_id['post_content'], 0, 140))).'"/>
		';
		}
		$image = 
		'
	...
	function dcssb_url_shortener($url){
		...
		if ((function_exists('curl_init') || function_exists('file_get_contents')) && function_exists('unserialize')) {
				...
				if ($short_url) {
					add_post_meta($post->ID, 'dcssb_short_url', $short_url);
				}
		}
