...
use KAC\AdminBundle\Entity\User;
use KAC\AdminBundle\Entity\Contact;
use KAC\AdminBundle\Entity\ContactNumber;
use KAC\AdminBundle\Entity\ContactAddress;
use KAC\AdminBundle\Entity\ContactEmailAddress;
...
class CheckoutController extends Controller
{
	...
	public function ajaxCheckEmailAddressAction(Request $request)
    {
		...
		$userObject = $em->getRepository('KACAdminBundle:User')->findOneBy(array('emailAddress' => $emailAddress));
		if ($userObject)
		{
	...
	public function ajaxGetOrderInformationAction(Request $request)
    {
		...
		return $this->render('KACShopBundle:Checkout:ajaxGetOrderInformation.html.twig', array('basket' => $basket, 'order' => $order));
    }
	...
	public function orderPlacedAction(Request $request, $paymentType)
    {
    	...
    	$orderObject = $em->getRepository('KACAdminBundle:Order')->find($orderId);
    	if (!$orderObject)
    	{
    		// Set error message
    	...
    	switch($paymentType)
    	{
    		...
    		case 'SagePay':
    			...
    			$sagePayService = $this->get('kac_admin.sage_pay_service');
...
//				}
		...
		if ($orderObject->getMembershipCardNumber() == 1)
		{
			...
			$membershipCardObject = $em->getRepository('KACAdminBundle:MembershipCard')->findOneBy(array('userId' => $orderObject->getUserId(), 'active' => 1));
			if ($membershipCardObject)
			{
				...
				$membershipCardObject = $em->getRepository('KACAdminBundle:MembershipCard')->findOneBy(array('userId' => '0', 'active' => 1));
				if ($membershipCardObject)
				{
		    		...
		    		try
					{
			        	...
			        	$email->setBody($this->renderView('KACShopBundle:Checkout:membershipCardInformation.html.twig', array('firstName' => $orderObject->getFirstName(), 'membershipNumber' => $membershipCardObject->getMembershipNumber())), 'text/html');
						$email->addPart($this->renderView('KACShopBundle:Checkout:membershipCardInformation.txt.twig', array('firstName' => $orderObject->getFirstName(), 'membershipNumber' => $membershipCardObject->getMembershipNumber())), 'text/plain');
			...
			$membershipCardObject = $em->getRepository('KACAdminBundle:MembershipCard')->findOneBy(array('membershipNumber' => $orderObject->getMembershipCardNumber(), 'active' => 1));
			if ($membershipCardObject)
			{
    	...
    	return $this->render('KACShopBundle:Checkout:orderPlaced.html.twig', array('order' => $order));
    }
    ...
    public function sendInvoice($id)
    {
		...
		try
		{
        	...
        	$email->setBody($this->renderView('KACShopBundle:Checkout:invoice.html.twig', array('order' => $order)), 'text/html');
			$email->addPart($this->renderView('KACShopBundle:Checkout:invoice.txt.twig', array('order' => $order)), 'text/plain');
