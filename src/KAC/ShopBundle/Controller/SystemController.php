...
class SystemController extends Controller
{
	...
	public function pageRequestAction(Request $request, $url, $results = 0, $page = 0, $sort = 0, $order = 0, $brand = 0, $brandId = 0, $group = 0, $priceFrom = 0, $priceTo = 0, $brands = 0, $options = 0, $features = 0)
    {
	    ...
	    $seoService = $this->get('kac_admin.seo_service');
	    ...
	    $systemService->initialiseSession();
		   		...
		   		case 'products':
		   			switch ($request->query->get('mode'))
			   		{
				   		case 'search':
				   			...
				   			if ($mid)
				   			{
					   			$midRoutingObject = $this->getDoctrine()->getRepository('KACAdminBundle:BrandDescription')->findOneBy(array('brandId' => $mid, 'locale' => 'en'));
					   			if ($midRoutingObject)
					   			{
				   			...
				   			if ($cid)
				   			{
					   			$cidRoutingObject = $this->getDoctrine()->getRepository('KACAdminBundle:Routing')->findOneBy(array('objectType' => 'department', 'objectId' => $cid, 'locale' => 'en'));
					   			if ($cidRoutingObject)
					   			{
   		...
   		{
   			$this->resetProductSearch();
			return $this->forward('KACShopBundle:System:index');
   		}
   		...
   		$routingObject = $this->getDoctrine()->getRepository('KACAdminBundle:Routing')->findOneBy(array('url' => $url, 'locale' => 'en'));
   		if (!$routingObject)
   		{
   			$redirectObject = $this->getDoctrine()->getRepository('KACAdminBundle:Redirect')->findOneBy(array('redirectFrom' => $url));
   			if ($redirectObject)
   			{
   				$this->resetProductSearch();
   		...
   		if ($routingObject)
   		{
   			switch ($routingObject->getObjectType())
   			{
   				case 'brand':
   					$this->resetProductSearch();
   					return $this->forward('KACShopBundle:Brands:index', array('id' => $routingObject->getObjectId()));
   					break;
   				case 'department':
					...
					if ($brand)
					{
						...
						$brandRoutingObject = $this->getDoctrine()->getRepository('KACAdminBundle:Routing')->findOneBy(array('url' => $brand, 'objectType' => 'brand', 'locale' => 'en'));
					...
					if (!$features)
					{
   					...
   					$this->resetProductSearch();
   					return $this->forward('KACShopBundle:Departments:index', array('id' => $routingObject->getObjectId(), 'url' => $url, 'brand' => $brand, 'group' => $group));
   					break;
   				case 'product':
   					// Check the status of the product
   					$productIndexObject = $this->getDoctrine()->getRepository('KACAdminBundle:ProductIndex')->findOneBy(array('productId' => $routingObject->getObjectId(), 'locale' => 'en'));
   					if ($productIndexObject)
   					{
	   					if ($productIndexObject->getStatus() == 'a')
	   					{
		   					return $this->forward('KACShopBundle:Products:index', array('id' => $routingObject->getObjectId()));
	   					} else {
	   						$productToDepartmentObject = $this->getDoctrine()->getRepository('KACAdminBundle:ProductToDepartment')->findOneBy(array('productId' => $routingObject->getObjectId(), 'displayOrder' => '1'));
	   						$departmentRoutingObject = $this->getDoctrine()->getRepository('KACAdminBundle:Routing')->findOneBy(array('objectId' => $productToDepartmentObject->getDepartmentId(), 'objectType' => 'department', 'locale' => 'en'));
	   						if ($productToDepartmentObject && $departmentRoutingObject)
	   						{
		   						return $this->forward('KACShopBundle:Departments:index', array('id' => $productToDepartmentObject->getDepartmentId(), 'url' => $departmentRoutingObject->getUrl(), 'brand' => 0, 'group' => 0));
	   						} else {
						   		...
						   		return $this->forward('KACShopBundle:Products:productSearch', array('search' => $search));
	   						}
	   					}
   					} else {
	   					$search = $seoService->generateUrl($url);
	...
	public function productRedirectAction($id, $description)
    {
	    ...
	    $routingObject = $this->getDoctrine()->getRepository('KACAdminBundle:Routing')->findOneBy(array('objectId' => $id, 'objectType' => 'product', 'locale' => 'en'));
	    if ($routingObject)
	    {
	    	return $this->redirect($this->get('router')->generate('page_request', array('url' => $routingObject->getUrl())), 301);
	    }
	    	   		   		    	
        return $this->render('KACShopBundle:System:pageNotFound.html.twig', array());
    }
