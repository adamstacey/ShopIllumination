...
class BuildWrongCodesGoogleProductFeedCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('admin:build:wrongcodesgooglefeed')
            ->setDescription('Build the wrong codes google product feed')
            ->addArgument('host', InputArgument::REQUIRED, 'The host to be used for the URLs')
            ->addArgument('filename', InputArgument::OPTIONAL, 'The file to be written to. Path should be relative to the document root', 'web/wrong-codes-google-products.xml')
        ;
    }
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        while (($row = $result->next()) !== false) {
            ...
            foreach ($product->getVariants() as $variant)
            {
                ...
                foreach (explode(',', $variant->getAlternativeProductCodes()) as $productCode)
                {
                    ...
                    $xmlWriter->writeElement('g:mpn', trim($productCode));
