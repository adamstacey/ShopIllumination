...
class FixDepartmentCommand extends ContainerAwareCommand
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        $departments = $em->createQuery("SELECT d FROM WebIllumination\SiteBundle\Entity\Department d WHERE d.parent IS NULL ORDER BY d.displayOrder ASC")->execute();

        // Find next ID
        $lastId = $em->createQuery("SELECT d.id FROM WebIllumination\SiteBundle\Entity\Department d ORDER BY d.id DESC")->setMaxResults(1)->getSingleScalarResult();
