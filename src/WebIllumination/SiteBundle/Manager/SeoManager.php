...
//        $this->mailer = $mailer;
    ...
    public function updateRedirects($objectId, $objectType, $redirectFrom, $redirectTo)
    {
        ...
        $existingRedirects = $this->doctrine->getRepository('WebIllumination\SiteBundle\Entity\Redirect')->findByRedirectFrom($redirectTo);
        ...
        $existingRedirects = $this->doctrine->getRepository('WebIllumination\SiteBundle\Entity\Redirect')->findByRedirectTo($redirectFrom);
        foreach ($existingRedirects as $existingRedirect)
        {
    ...
    public function doesUrlExist($url = '')
    {
        if ($url != '')
        {
            $urlCheck = $this->doctrine->getRepository('WebIllumination\SiteBundle\Entity\Routing')->findOneByUrl($url);
            if ($urlCheck)
            {
                return true;
            }
