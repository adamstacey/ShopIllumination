...
use WebIllumination\AdminBundle\Entity\UserKey;
...
class SecurityController extends Controller
{
	...
	public function loginAction(Request $request)
    {
        ...
        return $this->render('WebIlluminationShopBundle:Security:login.html.twig', array('data' => $data));
    }
	...
	public function ajaxLoginAction(Request $request)
    {
    	...
    	if ($request->isXmlHttpRequest())
    	{
			...
			// Get the user
			$userObject = $em->getRepository('WebIlluminationAdminBundle:User')->findOneBy(array('emailAddress' => $emailAddress, 'active' => 1));
			if (!$userObject)
			{
    		...
    		$membershipCardObject = $em->getRepository('WebIlluminationAdminBundle:MembershipCard')->findOneBy(array('userId' => $userObject->getId()));
    		...
    		if ($contact)
    		{
	    		...
	    		$telephoneDaytimeObject = $em->getRepository('WebIlluminationAdminBundle:ContactNumber')->findOneBy(array('contactId' => $contact['id'], 'contactNumberTypeId' => '5'), array('displayOrder' => 'ASC'));
	    		if ($telephoneDaytimeObject)
	    		{
	    			$order['telephoneDaytime'] = $telephoneDaytimeObject->getNumber();
	    		}
	    		$telephoneEveningObject = $em->getRepository('WebIlluminationAdminBundle:ContactNumber')->findOneBy(array('contactId' => $contact['id'], 'contactNumberTypeId' => '6'), array('displayOrder' => 'ASC'));
	    		if ($telephoneEveningObject)
	    		{
	    			$order['telephoneEvening'] = $telephoneEveningObject->getNumber();
	    		}
	    		$mobileObject = $em->getRepository('WebIlluminationAdminBundle:ContactNumber')->findOneBy(array('contactId' => $contact['id'], 'contactNumberTypeId' => '2'), array('displayOrder' => 'ASC'));
	    		if ($mobileObject)
	    		{
	    			$order['mobile'] = $mobileObject->getNumber();
	    		}
	    		$billingAddressObject = $em->getRepository('WebIlluminationAdminBundle:ContactAddress')->findOneBy(array('contactId' => $contact['id'], 'contactAddressTypeId' => '4'), array('displayOrder' => 'ASC'));
	    		...
	    		$deliveryAddressObject = $em->getRepository('WebIlluminationAdminBundle:ContactAddress')->findOneBy(array('contactId' => $contact['id'], 'contactAddressTypeId' => '5'), array('displayOrder' => 'ASC'));
	...
	public function ajaxForgottenYourPasswordAction(Request $request)
    {
    	...
    	if ($request->isXmlHttpRequest())
    	{
    		...
    		try
			{
	        	...
	        	$email->setBody($this->renderView('WebIlluminationShopBundle:Security:resetPasswordEmail.html.twig', array('passwordResetLink' => $passwordResetLink)), 'text/html');
				$email->addPart($this->renderView('WebIlluminationShopBundle:Security:resetPasswordEmail.txt.twig', array('passwordResetLink' => $passwordResetLink)), 'text/plain');
	...
	public function registerAction(Request $request)
    {
        ...
        return $this->render('WebIlluminationShopBundle:Security:register.html.twig', array());
    }
    ...
    {	
    	...
    	// Create the user key object
		$userKeyObject = $em->getRepository('WebIlluminationAdminBundle:UserKey')->findOneBy(array('userKey' => $userKey));
		if ($userKeyObject)
		{
			if ($userKeyObject->getExpiry()->getTimestamp() >= strtotime(date("Y-m-d H:i:s")))
			{
				...
				// Get the user
				$userObject = $em->getRepository('WebIlluminationAdminBundle:User')->findOneBy(array('id' => $userKeyObject->getUserId(), 'active' => 1));
				if ($userObject)
				{				  	
				  	// Authenticate the user
		    		$token = new UsernamePasswordToken($userObject, null, 'shop', array('ROLE_CUSTOMER'));
