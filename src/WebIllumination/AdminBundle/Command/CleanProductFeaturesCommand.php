...
class CleanProductFeaturesCommand extends ContainerAwareCommand
{
    protected function configure()
    {
        $this
            ->setName('admin:cleanProductFeatures')
            ->setDescription('Cleans the product features.')
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
	    ...
	    $productFeatureTotal = sizeof($productFeatureObjects);
	    foreach ($productFeatureObjects as $productFeatureObject)
	    {
		    $existingProductFeature = trim(strtoupper($productFeatureObject->getProductFeature()));
		    $newProductFeature = $existingProductFeature;
		    
		    // Make lowercase
		    $newProductFeature = strtolower($newProductFeature);
		    
		    // Uppercase words
		    $newProductFeature = ucwords($newProductFeature);
		    
		    // Make the replacements
		    $newProductFeature = str_replace($commonWordsSearch, $commonWordsReplace, $newProductFeature);
		    $newProductFeature = preg_replace($search, $replace, $newProductFeature);
		    $newProductFeature = str_replace('**not Set ***', '*** NOT SET ***', $newProductFeature);
		    		    
		    // Output		    
		    $output->writeln($productFeatureCount.' of '.$productFeatureTotal.': Feature Group changed from "<fg=yellow;options=bold>'.$existingProductFeature.'</fg=yellow;options=bold>" to "<fg=green;options=bold>'.$newProductFeature.'</fg=green;options=bold>"');
