...
class BasketService {
    ...
    public function initialiseBasketSession()
    {
        ...
        if (!is_array($basket))
        {
            ...
            if (!is_array($basket['voucherCodes']))
            {
    ...
    public function getDeliveryDetails($additionalDeliveryBand = 0)
    {
        ...
        $weight = 0;
        $band = 0;
        if (isset($basket['products']) && sizeof($basket['products']) > 0)
        {
        ...
        switch ($zone)
        {
            case 1:
                switch ($band)
                {
                    ...
                    case 2:
                        $deliveryOption = array();
                        $deliveryOption['service'] = 'FREE DELIVERY Parcel Economy';
    ...
    public function updateBasketTotals()
    {
        ...
        $voucherCodes = array("YOUSAVE", "USAVE");
        foreach ($basket['voucherCodes'] as $index => $voucherCode)
        {
            if ($voucherCode != '')
            {
                if (!in_array($voucherCode, $voucherCodes))
                {
                    $messages['error'][] = 'The voucher code <strong>"'.$voucherCode.'"</strong> does not exist. Please try another one.';
                    unset($basket['voucherCodes'][$index]);
                }
            }
        }
        ...
        if (isset($basket['products']) && sizeof($basket['products']) > 0)
        {
            // Reset all prices to before any discounts were set
            foreach ($basket['products'] as $product)
            {
        ...
        $uSaveDiscountApplied = false;
        if ($totalUSaveDiscount > 0)
        {
            $uSaveDiscountApplied = true;
        }
        $basket['uSaveDiscountApplied'] = $uSaveDiscountApplied;
