...
class SeoService {
    ...
    public function updateRedirects($objectId, $objectType, $redirectFrom, $redirectTo)
    {
        ...
        $existingRedirects = $doctrineService->getRepository('KAC\SiteBundle\Entity\Redirect')->findByRedirectTo($redirectFrom);
        foreach ($existingRedirects as $existingRedirect)
        {
        ...
        try
        {
            ...
            $email->setBody($this->container->get('templating')->renderResponse('WebIlluminationShopBundle:System:urlChange.html.twig', array('existingUrl' => $redirectFrom, 'url' => $redirectTo))->getContent(), 'text/html');
            $email->addPart($this->container->get('templating')->renderResponse('WebIlluminationShopBundle:System:urlChange.txt.twig', array('existingUrl' => $redirectFrom, 'url' => $redirectTo))->getContent(), 'text/plain');
    ...
    public function doesUrlExist($url = '')
    {
        if ($url != '')
        {
            $urlCheck = $this->container->get('doctrine')->getRepository('KAC\SiteBundle\Entity\Routing')->findOneByUrl($url);
            if ($urlCheck)
            {
                return true;
            }
