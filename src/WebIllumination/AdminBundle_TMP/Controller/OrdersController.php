...
use WebIllumination\SiteBundle\Entity\Order\Note as OrderNote;
use WebIllumination\SiteBundle\Entity\Order\Product as OrderProduct;
...
class OrdersController extends Controller
{
    ...
    public function indexAction(Request $request, $document = '')
    {
        ...
        if (isset($sessionListing['admin']['Order']))
        {
            $listing = $sessionListing['admin']['Order'];
            $this->listing = $listing;
        } else {
            $sessionListing['admin']['Order'] = $this->listing;
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if ($extraAction != '')
            {
                        ...
                        if (sizeof($select) > 0)
                        {
                            if (sizeof($select) == 1)
                            {
                                foreach ($select as $itemId => $item)
                                {
                                    ...
                                    $itemObject = $em->getRepository('WebIllumination\SiteBundle\Entity\Order')->find($itemId);
                                    if ($itemObject)
                                    {
                                    ...
                                    {
                                        return $this->redirect($this->get('router')->generate('admin_orders', array('document' => $document)));
                                    } else {
                            ...
                            // Notify user
                            $this->get('session')->getFlashBag()->add('notice', 'You did not select any orders to print.');
                ...
                if (sizeof($select) > 0)
                {
                    ...
                    // Notify user
                    $this->get('session')->getFlashBag()->add('notice', 'You did not select any orders to update.');
                ...
                if ($delete > 0)
                {
                    $this->get('session')->getFlashBag()->add('success', 'The selected orders have been deleted.');
                } else {
                    $this->get('session')->getFlashBag()->add('success', 'The selected orders have been updated.');
                }
        ...
        if (isset($sessionFilter['admin']['Order']))
        {
            $filter = $sessionFilter['admin']['Order'];
            $this->filter = $filter;
        } else {
            $sessionFilter['admin']['Order'] = $this->filter;
        ...
        $qb->from('WebIlluminationAdminBundle:Order', 'i');
        if ($this->filter['id'])
        {
    ...
    public function updateListingAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            $sessionListing = $this->get('session')->get('listing');
            $sessionListing['admin']['Order']['search'] = $search;
            $sessionListing['admin']['Order']['sortOrder'] = $sortOrder;
            $sessionListing['admin']['Order']['sort'] = $sort;
            $sessionListing['admin']['Order']['order'] = $order;
            $sessionListing['admin']['Order']['maxResults'] = $maxResults;
            $sessionListing['admin']['Order']['currentPage'] = $currentPage;
            $this->get('session')->set('listing', $sessionListing);
            ...
            $sessionFilter = $this->get('session')->get('filter');
            $sessionFilter['admin']['Order']['empty'] = 1;
            foreach ($filters as $index => $filter)
            {
                if ($index != 'empty')
                {
                    $filter = trim($filter);
                    $sessionFilter['admin']['Order'][$index] = $filter;
                    if ($filter != '')
                    {
                        $emptyFilters = false;
                    }
            ...
            if (!$emptyFilters)
            {
                $sessionFilter['admin']['Order']['empty'] = 0;
            }
    ...
    public function ajaxGetCustomerAction(Request $request)
    {
        ...
        // Get the item
        $itemObject = $em->getRepository('WebIllumination\SiteBundle\Entity\Order')->find($id);
    ...
    public function ajaxGetNotesAction(Request $request)
    {
        ...
        // Get the notes
        $itemNoteObjects = $em->getRepository('WebIllumination\SiteBundle\Entity\OrderNote')->findBy(array('orderId' => $id));
    ...
    public function ajaxGetProductsAction(Request $request)
    {
        ...
        // Get the products
        $data['products'] = $itemObject->getProducts();

        // Get the donations
        $data['donations'] = $itemObject->getDonations();
    ...
    public function printOrdersAction(Request $request)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if (sizeof($select) > 0)
            {
                ...
                foreach ($select as $itemId => $item)
                {
                    $orders[] = $this->getDoctrine()->getManager()->getRepository('WebIllumination\SiteBundle\Entity\Order')->find($itemId);
                }
                $orderHtml = $this->render('WebIlluminationAdminBundle:Orders:viewOrders.html.twig', array('orders' => $orders));
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if (sizeof($select) > 0)
            {
                ...
                if ($dpdImportFile != "Account|AddressCode|Name|Address 1|Address 2|Town|County|PostCode|Service|Qty of Labels|Contact|Telephone|Email|Email2|Additional Info\n")
                {
                    ...
                    // Notify user
                    $this->get('session')->getFlashBag()->add('success', 'The selected orders have been updated and '.sizeof($select).' DPD '.(sizeof($select) == 1?'order has':'orders have').' have been sent to the label printer.');

                    // Forward
                    return $this->redirect($this->get('router')->generate('admin_orders_process_deliveries'));
                } else {
                    // Notify user
                    $this->get('session')->getFlashBag()->add('success', 'The selected orders have been updated.');
        ...
        if (file_exists($trackingFileName))
        {
            ...
            foreach (explode("\n", $trackingFileContents) as $line)
            {
                ...
                if (sizeof($orderInfo) > 1)
                {
                    ...
                    // Get the item
                    $itemObject = $em->getRepository('WebIllumination\SiteBundle\Entity\Order')->find($orderId);
                    if ($itemObject)
                    {
    ...
    public function updateAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            {
                // Notify user
                $this->get('session')->getFlashBag()->add('error', 'Sorry, there was a problem saving the order <strong>"'.$itemObject->getId().'"</strong>. Please try again.');

                // Forward
                return $this->redirect($this->get('router')->generate('admin_orders_update', array('id' => $id)));
            }
            ...
            // Notify user
            $this->get('session')->getFlashBag()->add('success', 'The order <strong>"'.$itemObject->getId().'"</strong> has been updated.');
        ...
        $data['settings'] = $this->settings;
        $data['item'] = $itemObject;
        $data['formAction'] = $this->get('router')->generate('admin_orders_update', array('id' => $id));
        $data['mode'] = 'update';
    ...
    public function updatePaymentAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if ($goBack > 0)
            {
                ...
                return $this->redirect($this->get('router')->generate('admin_orders_update_payment', array('id' => $id)));
            }
        }
        ...
        $data['settings'] = $this->settings;
        $data['item'] = $itemObject;
        $data['formAction'] = $this->get('router')->generate('admin_orders_update_payment', array('id' => $id));
        $data['mode'] = 'update';

        // Get the items
        $data['items'] = $itemObject->getNotes();
    ...
    public function updateDeliveryAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if ($goBack > 0)
            {
                ...
                return $this->redirect($this->get('router')->generate('admin_orders_update_delivery', array('id' => $id)));
            }
        }
        ...
        $data['settings'] = $this->settings;
        $data['item'] = $itemObject;
        $data['formAction'] = $this->get('router')->generate('admin_orders_update_delivery', array('id' => $id));
        $data['mode'] = 'update';
        $data['items'] = $itemObject->getNotes();
    ...
    public function updateProductsAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if (sizeof($select) > 0)
            {
                ...
                {
                    // Get the item
                    $itemProductObject = $itemObject->getProducts();
            ...
            if($newItem)
            {
                /**
                 * Find product
                 * @var \WebIllumination\SiteBundle\Entity\Product $product
                 */
                if (!$newProductId || !($product = $em->getRepository('WebIllumination\SiteBundle\Entity\Product')->find($newProductId)))
                {
                    // Notify user
                ...
                $itemProductObject->setProductId($product->getId());
                $itemProductObject->setProduct($product);
                $itemObject->addProduct($itemProductObject);

                $prices = $product->getPrices();
                if(count($prices) > 0) {
                    /**
                     * @var \WebIllumination\SiteBundle\Entity\Product\Price $price
                     */
                    $price = $prices[0];

                    $itemProductObject->setVat($itemObject->getVat());
                    $itemProductObject->setSubTotal($price->getListPrice() * $newProductQuantity);
                }
            ...
            foreach($itemObject->getProducts() as $product)
            {
            ...
            if ($goBack > 0)
            {
                ...
                return $this->redirect($this->get('router')->generate('admin_orders_update_products', array('id' => $id)));
            }
        }
        ...
        $data['formAction'] = $this->get('router')->generate('admin_orders_update_products', array('id' => $id));
        $data['mode'] = 'update';
        $data['items'] = $itemObject->getProducts();
    ...
    public function updateNotesAction(Request $request, $id)
    {
        ...
        if ($request->getMethod() == 'POST')
        {
            ...
            if (sizeof($select) > 0)
            {
                foreach ($select as $itemId => $item)
                {
                     ...
                     */
                    $itemNoteObject = $em->getRepository('WebIllumination\SiteBundle\Entity\OrderNote')->find($itemId);
            ...
            if (sizeof($addItem) > 0)
            {
                ...
                foreach ($addItem as $itemId => $item)
                {
                    ...
                    if ($itemNote)
                    {
                        ...
                        $itemObject->addNote($itemNoteObject);
                        $em->persist($itemNoteObject);
                        $em->flush();
                ...
                // Update items count
                $itemObject->setNotesCount(count($itemObject->getNotes()));
                $em->persist($itemObject);
                $em->flush();
            }
            ...
            if ($goBack > 0)
            {
                ...
                return $this->redirect($this->get('router')->generate('admin_orders_update_notes', array('id' => $id)));
            }
        }
        ...
        $data['settings'] = $this->settings;
        $data['item'] = $itemObject;
        $data['formAction'] = $this->get('router')->generate('admin_orders_update_notes', array('id' => $id));
        $data['mode'] = 'update';

        // Get the items
        $data['items'] = $em->getRepository('WebIllumination\SiteBundle\Entity\OrderNote')->findBy(array('orderId' => $id), array('createdAt' => 'DESC'));
    ...
    public function viewCopyOrderAction(Request $request, $id)
    {
        ...
        // Get the order
        $item = $this->getDoctrine()->getManager()->getRepository('WebIllumination\SiteBundle\Entity\Order')->find($id);
